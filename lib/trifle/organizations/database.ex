defmodule Trifle.Organizations.Database do
    ...
    parsed_granularities =
      case granularities_input do
        nil ->
          ...
          Logger.debug(
            "parse_granularities: attrs=#{inspect(attrs)}, existing=#{inspect(existing_granularities)}"
          )
  ...
  def check_status(database) do
    try do
      ...
      Logger.info(
        "Setting database status to: #{status} (setup_exists: #{setup_exists}, error: #{inspect(error_msg)})"
      )
        ...
        "mongo" ->
          ...
          case Mongo.start_link(url: url) do
            {:ok, conn} ->
              ...
              Logger.info(
                "MongoDB calling setup! with collection: #{collection_name}, joined: #{joined_identifiers}, expire: #{inspect(expire_after)}"
              )
        ...
        result =
          try do
            ...
            case Postgrex.query(conn, query, [table_name]) do
              ...
              other ->
                Logger.error(
                  "PostgreSQL table check returned unexpected result: #{inspect(other)}"
                )
