defmodule TrifleApp.Components.FilterBar do
  ...
  def render(assigns) do
    ...
    <!-- Dropdown (visible on small screens or when forced by parent) -->
            <div class={
              ((@force_granularity_dropdown || @show_granularity_dropdown) && "block") ||
                "block md:hidden"
            }>
              <div class="relative">
                <div class="absolute -top-2 left-2 inline-block bg-white dark:bg-slate-800 px-1 text-xs font-medium text-gray-900 dark:text-white z-10">
  ...
  def update(assigns, socket) do
    ...
    is_navigation_update =
      (current_from != new_from || current_to != new_to) &&
        socket.assigns[:current_input_value] != nil

    current_input =
      if is_navigation_update do
        # Navigation controls changed the timeframe - update the input display
        get_input_value(
          assigns.smart_timeframe_input,
          assigns.use_fixed_display,
          assigns.from,
          assigns.to
        )
      else
        # Normal update - preserve user typing
        socket.assigns[:current_input_value] ||
          get_input_value(
            assigns.smart_timeframe_input,
            assigns.use_fixed_display,
            assigns.from,
  ...
  def handle_event("toggle_granularity_dropdown", _params, socket) do
    {:noreply,
     assign(socket, show_granularity_dropdown: !socket.assigns.show_granularity_dropdown)}
  end
