defmodule TrifleApp.Components.DashboardWidgets.CategoryEditor do
  ...
  def editor(assigns) do
    ...
    rows = Helpers.chart_path_rows(widget, "category")
              ...
              <div class="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_12rem_auto] gap-2 lg:items-start">
                <div class="min-w-0">
                  <.path_autocomplete_input
                    id={"widget-cat-path-#{Map.get(@widget, "id")}-#{row.index}"}
                    name="cat_paths[]"
                ...
                <div>
                  <SeriesColorSelector.input
                    id_prefix={"widget-cat-color-#{Map.get(@widget, "id")}"}
                    name="cat_color_selector"
                    index={row.index}
