defmodule TrifleApp.TimeframeParsing do
  ...
  def format_smart_timeframe_display(smart_input, _config, _, _, _)
      when smart_input in ["", nil] do
    ""
  end
  ...
  def get_timezone_offset_display(timezone) do
    try do
      ...
      offset_seconds =
        case DateTime.to_unix(tz_now) - DateTime.to_unix(now) do
          0 -> 0
  ...
  def find_short_code_from_description(input) do
    preset_map =
      Enum.into(timeframe_presets(), %{}, fn {short_code, description} ->
  ...
  def get_smart_timeframe_description(smart_input),
    do: generate_description_from_shorthand(smart_input)
