defmodule TrifleApp.ExploreLive do
  ...
  import TrifleApp.Components.DashboardFooter,
    only: [dashboard_footer: 1, transponder_errors_modal: 1]
  ...
  def mount(params, session, socket) do
    params
    |> ExploreCore.mount(session, socket)
    |> ensure_explore_path()
  end

  defp ensure_explore_path({:ok, socket}) do
  ...
  def handle_event(event, params, socket), do: ExploreCore.handle_event(event, params, socket)

  @impl true
  def handle_async(task, result, socket) do
    ExploreCore.handle_async(task, result, socket)
  end
  ...
  def render(assigns) do
    summary = ExploreCore.get_summary_stats(assigns)
    ...
    items =
      ...
      |> Enum.map(fn {key, value} ->
        ...
        %{
          ...
          label: key,
          value: numeric_value,
          formatted_value: ExploreCore.format_number(numeric_value),
          color: ExploreCore.get_key_color(keys_map, key)
        }
      end)
  ...
  defp explore_path, do: ~p"/explore"
end
