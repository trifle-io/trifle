defmodule TrifleApp.OrganizationLive do
  ...
  def handle_params(
        %{"tab" => tab_param},
        _uri,
        %{assigns: %{current_membership: %OrganizationMembership{}}} = socket
      ) do
      ...
      <%= if @current_membership do %>
        <div class="mb-6 border-b border-gray-200 dark:border-slate-700">
          <nav class="-mb-px flex flex-wrap gap-4" aria-label="Organization tabs">
            <.link
              navigate={~p"/organization?tab=profile"}
              class={tab_link_classes(@active_tab == :profile)}
            >
              <svg
            ...
            <.link
              navigate={~p"/organization?tab=users"}
              class={tab_link_classes(@active_tab == :users)}
            >
              <svg
            ...
            <.link
              navigate={~p"/organization?tab=invitations"}
              class={tab_link_classes(@active_tab == :invitations)}
            >
              <svg
                class={tab_icon_classes(@active_tab == :invitations)}
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                stroke-width="1.5"
            ...
            <.link
              navigate={~p"/organization?tab=billing"}
  ...
  def handle_event("resend_invitation", %{"id" => invitation_id}, socket) do
    with %OrganizationInvitation{} = invitation <-
           find_invitation(socket.assigns.invitations, invitation_id),
          ...
          <div class="px-6 py-4 flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
              <%= if @can_manage and member.id != @current_membership.id do %>
                ...
                <select
                  class="block w-32 rounded-md border-gray-300 dark:border-slate-700 text-sm shadow-sm focus:border-teal-500 focus:ring-teal-500 dark:bg-slate-900 dark:text-white"
                  name="role"
      ...
      <div class="lg:col-span-2 bg-white dark:bg-slate-800 shadow-sm border border-gray-200 dark:border-slate-700 rounded-lg">
        <div class="px-6 py-4 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-gray-500 dark:text-slate-400">
              Pending invites expire 3 days after creation.
            </p>
          </div>
        </div>
        <div class="divide-y divide-gray-200 dark:divide-slate-700">
          <%= for invitation <- @invitations do %>
            <div class="px-6 py-4 flex items-center justify-between">
              <div>
                ...
                <div class="text-xs text-gray-500 dark:text-slate-400">
                  Role: {String.capitalize(invitation.role)} • Status: {String.capitalize(
                    invitation.status
          ...
          <.form for={@invitation_form} phx-submit="invite_member" class="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                class="mt-1 block w-full rounded-md border-gray-300 dark:border-slate-700 text-sm shadow-sm focus:border-teal-500 focus:ring-teal-500 dark:bg-slate-900 dark:text-white"
              >
                <%= for role <- @roles do %>
