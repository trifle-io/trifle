defmodule TrifleApp.DatabaseTranspondersLive do
  ...
  def handle_info({TrifleApp.DatabaseTranspondersLive.FormComponent, {:updated, transponder}}, socket) do
    {:noreply, stream_insert(socket, :transponders, transponder)}
  end
  ...
  def render(assigns) do
    ...
    <div class="flex flex-col dark:bg-slate-900 min-h-screen">
          ...
          <div class="divide-y divide-gray-100 dark:divide-slate-700">
            <%= if @streams.transponders.inserts == [] do %>
              <div class="py-12 text-center">
                ...
                <h3 class="mt-2 text-sm font-semibold text-gray-900 dark:text-white">No transponders</h3>
                <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">Get started by creating a new transponder.</p>
              </div>
            <% else %>
              <div
                id="transponders"
                      ...
                      <div class="flex items-center gap-2">
                        ...
                        <button
                          ...
                          class={[
                            ...
                            if(transponder.enabled, do: "bg-teal-600", else: "bg-gray-200 dark:bg-slate-600")
                          ]}
                        >
      ...
      <!-- Modals -->
      <.app_modal :if={@live_action in [:new, :edit]} id="transponder-modal" show on_cancel={JS.patch(~p"/app/dbs/#{@database.id}/transponders")}>
        ...
        <:body>
          <.live_component
            ...
            database={@database}
            patch={~p"/app/dbs/#{@database.id}/transponders"}
          />
        </:body>
      </.app_modal>

      <.app_modal :if={@live_action == :show} id="transponder-details-modal" show on_cancel={JS.patch(~p"/app/dbs/#{@database.id}/transponders")}>
