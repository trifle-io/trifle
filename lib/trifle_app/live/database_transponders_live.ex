defmodule TrifleApp.DatabaseTranspondersLive do
  ...
  def render(assigns) do
    ...
    <div class="flex flex-col dark:bg-slate-900 min-h-screen">
      ...
      <!-- Modals -->
      <.modal :if={@live_action in [:new, :edit]} id="transponder-modal" show on_cancel={JS.patch(~p"/app/dbs/#{@database.id}/transponders")}>
        <.live_component
      ...
      <.modal :if={@live_action == :show} id="transponder-details-modal" show on_cancel={JS.patch(~p"/app/dbs/#{@database.id}/transponders")}>
        <.live_component
