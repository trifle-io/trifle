defmodule TrifleApp.DashboardLive do
  ...
  defp apply_action(socket, :configure, _params) do
    ...
    |> assign_dashboard_template_options(socket.assigns.dashboard)
  end
  ...
  def handle_event("dashboard_grid_changed", %{"items" => items}, socket) do
    cond do
      ...
      !socket.assigns.can_edit_dashboard_layout ->
        {:noreply, socket}
      ...
      true ->
        ...
        case Organizations.update_dashboard_for_membership(
          ...
          {:error, reason} ->
            {:noreply, handle_layout_update_error(socket, reason)}
        end
    end
  end
      ...
      true ->
        with id when not is_nil(id) <- param(params, "widget_id"),
          ...
          updated =
            ...
            |> then(&apply_widget_params_for_edit(widget, &1, path_options))
  ...
  def handle_event(
    ...
    cond do
      ...
      true ->
        rows = MetricSeries.normalize_event_rows(raw_rows)
  ...
  def handle_event(
    ...
    cond do
      ...
      template_id == socket.assigns.dashboard.template_id ->
      ...
      true ->
        ...
        case Organizations.link_dashboard_template(dashboard, membership, template_id) do
          {:ok, updated_dashboard} ->
            updated_socket =
              socket
          ...
          {:error, reason} ->
            {:reply, %{ok: false},
             put_flash(socket, :error, dashboard_template_error_message(reason))}
        end
    end
  end
  ...
  def handle_event("segments_editor_change", params, socket) do
    ...
    socket =
      ...
      |> assign(
        :selected_dashboard_template_id,
        Map.get(
          params,
          "template_id",
          socket.assigns[:selected_dashboard_template_id]
        )
      )
  ...
  def handle_event("convert_dashboard_to_template", params, socket) do
    cond do
      !socket.assigns.can_edit_dashboard ->
        {:noreply,
         put_flash(
      ...
      !is_nil(socket.assigns.dashboard.template_id) ->
        {:noreply, put_flash(socket, :error, "This dashboard already uses a template")}
      ...
      true ->
        ...
        case Organizations.convert_dashboard_to_template(
  ...
  def handle_event("detach_dashboard_template", _params, socket) do
    if !socket.assigns.can_edit_dashboard do
      {:noreply,
       put_flash(
         socket,
      ...
      case Organizations.detach_dashboard_template(dashboard, membership) do
        {:ok, updated_dashboard} ->
          {:noreply,
           socket
           |> assign_dashboard(updated_dashboard)
  ...
  def handle_event("save_settings", params, socket) do
    if !socket.assigns.can_edit_dashboard do
      ...
      template_id = Map.get(params, "template_id", dashboard.template_id)
      ...
      with {:ok, source} <-
        ...
        case Organizations.update_dashboard_configuration(
               dashboard,
               membership,
               attrs,
               template_id
  ...
  defp dashboard_chat_context(socket) do
    ...
    ChatPageContext.build(:dashboard,
      ...
      capabilities: %{
        ...
        can_apply_dashboard_update: socket.assigns[:can_edit_dashboard_layout] == true
      },
      editable_payload_ref:
        if(socket.assigns[:can_edit_dashboard_layout],
  ...
  defp maybe_auto_expand_widget_paths(widget, _options) when is_map(widget) do
    normalized = Registry.normalize_widget(widget)

    if MetricSeries.metric_widget?(normalized) do
      MetricSeries.prune_legacy_metric_fields(normalized)
    else
      normalized
    end
  end
  ...
  defp assign_dashboard_permissions(socket) do
    ...
    cond do
      ...
      true ->
        ...
        |> assign(:can_edit_dashboard_layout, false)
  ...
  defp assign_dashboard(socket, dashboard) do
    dashboard = normalize_dashboard_metric_widgets(dashboard)
  ...
  defp assign_dashboard_template_options(socket, dashboard) do
    socket
    |> assign(
      :dashboard_template_groups,
      Organizations.list_available_dashboard_templates(socket.assigns.current_membership)
    )
    |> assign(:selected_dashboard_template_id, dashboard.template_id)
  end

  defp dashboard_template_error_message(:already_template_backed),
    do: "This dashboard already uses a template"

  defp dashboard_template_error_message(:template_not_found),
    do: "The selected dashboard template is no longer available"

  defp dashboard_template_error_message(:invalid_template_id),
    do: "The selected dashboard template is invalid"

  defp dashboard_template_error_message(:template_detach_requires_context),
    do: "Use the Detach action to stop using this template"

  defp dashboard_template_error_message(:stale_template),
    do: "This dashboard template changed. Reload the page and try again."

  defp dashboard_template_error_message(%Ecto.Changeset{}),
    do: "Failed to save settings"

  defp dashboard_template_error_message(_reason), do: "Failed to update the dashboard template"
  ...
  defp handle_layout_update_error(socket, :template_read_only) do
    put_flash(socket, :error, "System template layouts cannot be edited.")
  end

  defp handle_layout_update_error(socket, _reason) do
    put_flash(socket, :error, "Failed to update the dashboard layout.")
  end

  defp normalize_dashboard_metric_widgets(%_{} = dashboard) do
    payload = Map.get(dashboard, :payload, %{}) || %{}
    normalized_payload = normalize_dashboard_payload(payload)
    %{dashboard | payload: normalized_payload}
  end

  defp normalize_dashboard_metric_widgets(dashboard) when is_map(dashboard) do
    Map.update(dashboard, :payload, %{}, &normalize_dashboard_payload/1)
  end

  defp normalize_dashboard_metric_widgets(other), do: other

  defp normalize_dashboard_payload(payload) when is_map(payload) do
    grid =
      payload
  ...
  defp normalize_dashboard_payload(other), do: other

  defp normalize_dashboard_widget_item(item) when is_map(item) do
    item = Registry.normalize_widget(item)

    if MetricSeries.metric_widget?(item) do
      MetricSeries.prune_legacy_metric_fields(item)
    else
      item
    end
  end

  defp normalize_dashboard_widget_item(other), do: other
  ...
  defp build_distribution_widget(
    ...
    series_rows =
      Keyword.get_lazy(opts, :series_rows, fn ->
        metric_series_rows_from_params(params, source_widget)
      end)
    ...
    fallback_heatmap_color = heatmap_single_color_from_series(series_rows)
  ...
  defp metric_series_rows_from_params(params, source_widget, opts \\ []) do
    if metric_series_row_params_present?(params) do
      MetricSeries.normalize_series_rows_params(params, opts)
    else
      source_widget
      |> normalize_metric_series_source_widget(opts)
      |> Map.get("series", [])
      |> MetricSeries.normalize_series_rows(opts)
    end
  end

  defp normalize_metric_series_source_widget(source_widget, opts) do
    if Keyword.get(opts, :preserve_empty, false) do
      MetricSeries.normalize_widget_for_form(source_widget)
    else
      MetricSeries.normalize_widget(source_widget)
    end
  end

  defp metric_series_row_params_present?(params) when is_map(params) do
    Enum.any?(
      [
        "widget_series_kind",
        "widget_series_path",
  ...
  defp metric_series_row_params_present?(_), do: false

  defp strip_metric_series_change_params(params) when is_map(params) do
    Enum.reduce(
      [
        "widget_series_kind",
        "widget_series_path",
  ...
  defp strip_metric_series_change_params(params), do: params

  defp heatmap_single_color_from_series(rows) when is_list(rows) do
    selector =
      rows
      |> Enum.find_value(fn row ->
        path = row |> Map.get("path", "") |> to_string() |> String.trim()

        if path == "" do
          nil
        else
          MetricSeries.row_color_selector(row)
        end
      end)
      |> Kernel.||(DashboardWidgetHelpers.default_series_color_selector())

    DashboardWidgetHelpers.resolve_series_color(selector, 0)
  end
  ...
  defp open_widget_workspace(socket, id, requested_tab) do
    editable? =
      !socket.assigns.is_public_access and
        ...
        socket.assigns.can_edit_dashboard_layout and
