defmodule TrifleApp.DesignSystem.DataTable do
  ...
  def table_header(assigns) do
    ~H"""
    <div class={["py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 dark:text-white sm:pl-3 border-b border-gray-100 dark:border-slate-700 flex items-center justify-between", @class]}>
  ...
  def table_summary(assigns) do
    ~H"""
    <div class={["border-t border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-800 px-4 py-3", @class]}>
  ...
  def summary_stat(assigns) do
    ...
    <div class={["flex items-center gap-1", @class]}>
      ...
      <%= if @clickable && @phx_click do %>
        <button phx-click={@phx_click} class="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 underline">
          {@value}
        </button>
      <% else %>
