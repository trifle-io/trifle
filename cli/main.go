...
func metricsGet(args []string) {
  ...
  from := fs.String("from", "", "ISO8601 start timestamp")
  to := fs.String("to", "", "ISO8601 end timestamp")
  granularity := fs.String("granularity", "", "Granularity (e.g. 1h)")
  fs.Parse(args)
  ...
  if granularityValue == "" {
    granularityValue, err = resolveGranularity(context.Background(), client)
    if err != nil {
      exitError(err)
    }
  }
...
func metricsPush(args []string) {
  ...
  opts := addCommonFlags(fs)
  key := fs.String("key", "", "Metrics key")
  at := fs.String("at", "", "ISO8601 timestamp (default: now)")
