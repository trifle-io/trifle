defmodule Trifle.Chat.ProgressTest do
  ...
  test "received exposes an immediate waiting state" do
    assert Progress.text(:received, %{}) == "Waiting for AI response."
    assert Progress.text("received", %{}) == "Waiting for AI response."
  end

  test "thinking labels come from the cake-prep list" do
    assert Progress.random_thinking_label() in Progress.thinking_labels()
  end

  test "thinking uses the provided cake-prep label" do
    assert Progress.text(:thinking, %{label: "Whisking"}) == "Whisking."
    assert Progress.text("thinking", %{"label" => "Frosting"}) == "Frosting."
  end

  test "tool_error includes a readable reason" do
    assert Progress.text(:tool_error, %{
             tool: "build_metric_dashboard",
             reason: "grid must contain at least one widget"
           }) ==
  ...
  test "tool_error safely formats non string-char terms" do
    text =
      Progress.text(:tool_error, %{
        tool: "build_metric_dashboard",
        reason: {:bad_input, [path: "metrics"]}
      })
  ...
  test "inspecting_metric_schema reports the schema sample descriptor" do
    assert Progress.text(:inspecting_metric_schema, %{
             metric_key: "sales",
             timeframe: "schema sample",
             granularity: "1d"
           }) ==
  ...
  test "fetching_timeseries reports safe granularity adjustments" do
    assert Progress.text(:fetching_timeseries, %{
             metric_key: "sales",
             timeframe: "60d",
             granularity: "1d",
             adjusted_from: "1m"
  ...
  test "formatting_series reports dashboard granularity adjustments" do
    assert Progress.text(:formatting_series, %{
             metric_key: "sales",
             formatter: "dashboard",
             timeframe: "60d",
             granularity: "1d",
